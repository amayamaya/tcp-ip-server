...
describe('The TCP server', () => {
  ...
  // working HTTP server.
  it.only('connects on the default port', async () => {
    await request(server).get('/').expect(200);
  });

  it('connects to the post route with JSON', async () => {
    await request(server).get('/posts').expect(200).expect('Content-Type', 'application/json')
  });
  ...
  it('receives a 204 when a post request is made', async () => {
    await request(server).post('/posts').expect(204).expect('Content-Type', 'application/json');
  });
